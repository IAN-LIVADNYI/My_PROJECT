...
const OneCompanyPage = require("../pageObjects/OneCompanyPage");
...
const ProblemCreatePage = require("../pageObjects/ProblemCreatePage");
...
describe("CompaniesPage-1", () => {
    ...
    it("CP-20: Verify user got back to the Companies Page", async () => {
        ...
        await browser.waitUntil(async () => await CompaniesPage.elemAnyCompanyCardXpath.isClickable() === true, {timeout:5000});
        const numberOfCards =  await browser.$$(CompaniesPage.anyCompanyCardXpath).length;
        await expect(numberOfCards === 9).toEqual(true);
    });
...
// describe("CompaniesPage-2", () => {
    ...
    //         await ProblemCreatePage.inputTitle.setValue(`Task${new Date().getMilliseconds()*1.33*new Date().getSeconds()}`);
    //         await ProblemCreatePage.inputCompanyName.click();
    //         await browser.$("//li[@id='company-option-0']").click();
    //         await ProblemCreatePage.inputPosition.setValue("The Best QA Engineer Ever");
    //         await ProblemCreatePage.inputContent.setValue(`Content for the Task # ${i}`);
    //         await ProblemCreatePage.btnSave.click();
    //         i++;
    //     }
    //     while (i <= 3);
    ...
    //     await expect(newNumber - number >= 3).toEqual(true);
    // })

    // it("CP-24: Delete created problems and verify they are deleted", async () => {
    //     const userId = (await apiMethods.loginUserViaAPI(email, password))._userId;
    //     const accessToken = (await apiMethods.loginUserViaAPI(email, password)).accessTok;
